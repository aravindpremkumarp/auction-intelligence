...
**Files:** Modify `scripts/load_tn_to_neo4j.py`, `scripts/upload_downloads_to_r2.py`, `scripts/init_graph_schema.py`.

- [ ] Loader: `--input` (glob, default `data/listings/*.jsonl`, fallback `data/tn_auction_data.jsonl`); `--dry-run` prints the first batch's rows; SET the new props; `MERGE (m:Media {url})` with `kind`, `is_main`, `source` and `(a)-[:HAS_MEDIA]->(m)`; insert filter becomes "has ≥1 document or ≥1 media"; backfill Cypher for `source="eauctionsindia"`, `source_rank=3` on untouched nodes, run once.
- [ ] R2 upload: search `downloads/<source>/`; set `doc.source`, `doc.doc_role` from the listing's document refs; mirror photos of live listings to `media/{auction_id}/{sha}.{ext}`, set `m.content_sha256`, `m.r2_key`, `m.public_url`; fix the docstring at `:122`.
- [ ] Schema: `auction_source_idx`, `auction_source_id_idx`, `event_id_unique`, `media_sha_idx`.
- [ ] Load both `--limit 20` files into a local Neo4j (the `e2e` job's `neo4j:5.26` container recipe) and check `MATCH (a:AuctionProperty) RETURN a.source, count(*)` shows three sources and the unique constraint holds with prefixed ids.
