...
- **Reads:** `GET /stats` (live counts) and `GET /properties` for post fodder — new/upcoming auctions, **price drops** (`is_reauction` + `previous_reserve_price`, and the cut must clear 5% or ₹2L — smaller moves fall through to another angle rather than leading a batch with a discount the figures can't cash), **cheapest-by-city** (`sort=price_asc`), "closing soon."
- **Writes:** 3–5 post drafts + one city carousel (its slides are picked from data by `select_carousel()`; the model writes only the cover hook and caption, gated by `validate_carousel()`) in our voice (brand block pulled from `product-marketing.md`; taboo list = the honesty rule). The copy craft — the hook system (stop test + 8 mechanisms + 3-variant discipline) + the quality bar — comes from **`docs/marketing/copy-playbook.md`** (translated from the `social`, `copywriting`, and `copy-editing` skills into auction-specific swipe lines); its essentials are injected into `build_prompt()`, and the objective gates (must contain a figure; no banned words; hook ≤100 chars; no throat-clearing openers; mechanism variety) are enforced in `validate_drafts()`. Optionally renders the visuals via the existing HTML pipeline (`brand/logo/render.py` + `web/card-variations.html` + `web/styles.css` tokens; reels via the `hyperframes` npm package).
- **On demand, one property:** `python -m marketing_agents.poster --prepare --auction-id <id>` (then generate + `--finalize`) stages the full kit for a single auction — its static card, its own 6-slide `property-carousel-1080x1350`, and a caption. Deliberately off in a normal batch: five drafts would add 30 carousel slides per run to a gate that publishes ~5 posts/week. It adds no new model output either — the carousel cover reuses the draft's `image_headline`, so it inherits that field's honesty scan and length cap.
