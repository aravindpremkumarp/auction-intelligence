...
**A hook can only be as strong as the candidate under it.** The gates in `validate_drafts()` are all one-directional — they drop copy for being *too loud* (banned wording, hype openers, an over-long hook) and nothing drops copy for being boring, because "boring" has no objective test. That asymmetry means candidate selection, not the prose rules, is where hook strength is actually won or lost. Three things follow from it:

- **A price drop needs a floor.** `_is_price_drop()` requires the cut to clear `MIN_DROP_PCT` (5%) **or** `MIN_DROP_ABS` (₹2L) — the absolute floor because a big-ticket lot can move under 5% and still be a real cut. Without it, any re-auction that moved a rupee was tagged `price_drop`, and since `shape_candidates()` ranks that angle first, an ₹87,000 trim on a ₹31.9L plot arrived as the batch's best contrast story: `₹31.9L → ₹31L`, two figures that read as the same number. Below the floor the lot is **not** discarded — it falls through to `closing_soon` / `cheapest`, which describe it honestly and don't hand the model a discount it can't cash.
- **The mechanism has to match the size of the fact.** `contrast` earns the first line only when the delta is visible at a glance (≈10%+). Under that, the *small* move is the story and `process` carries it — "the bank wanted ₹31.9L. nobody bid. today: ₹31L — ₹87,000 less." Same facts, real gap. The prompt states this as the MECHANISM FIT rule.
- **The arsenal follows the angles.** `pillars_for()` picks which `hooks.json` pillars the prompt carries: `deals` + `market_data` always, `education` when a price drop is in the pool, `evaluation` on a deadline, `geo` + `evaluation` for `cheapest`. It used to be hardcoded to two pillars, which hid 56 of the 78 curated hooks — including every `geo` and `evaluation` one, the pillars that fit the majority case of a plain live listing with no drop and no deadline. `news` / `qa` / `build_in_public` stay out: their hooks interpolate `{headline_short}`, `{question_short}`, `{total}`, fields a post draft has no source for and could only fill by inventing.

**One property is the subject; the group number is only the ruler.** A property post promises one lot, so its hook must be about that lot — "only 11 lots in kanchipuram are cheaper than this ₹27.8L flat" is a property hook, while "kanchipuram has 69 live auctions" is a *city* hook and belongs to the carousel, which is a city post. Get this backwards and the hook promises an inventory the post never delivers. `city_context()` computes the yardstick for each candidate against the live lots of its own city — `city_total`, `rank`, `cheaper`, `dearer`, `median_lakhs`, `vs_median_lakhs`, `cheaper_than_pct`, `area_count` — and the prompt fences it: quote those figures exactly, never round ("about 70" for 69), recompute, or infer new ones. Two deliberate limits: `fetch_market()` pages the **whole** live set rather than one 200-row page, because a ranking claim against a prefix is only a lower bound; and context is attached only when the lot is itself in that set and its city clears `MIN_CONTEXT_LOTS` — a rank out of three is arithmetic, not a story.

**`risk` is a base pillar, not an angle.** What the reserve price *doesn't* cover — arrears, dues, the "as is where is" clause, the balance window, the EMD you can lose — is true of every lot we list, with or without a drop or a deadline. It's also the format that saves best and the one only we can write honestly, so it ships with every batch instead of waiting for an angle to summon it. `guess` joins the mechanism list as the ninth: state the real figure in the body, because a guess hook that never answers is bait and the answer is public data anyway.

**"Calm" is a constraint on words, not on tension.** The brand block ("understated, plain-spoken, lowercase, calm, no hype") is injected at the top of the prompt and the quality bar repeats it at the bottom, so a model reading conservatively concludes that a flat opening line is the compliant one. It isn't: "3 days left. nobody has checked the flood map." is calm *and* stops the scroll. The prompt now says so explicitly — if a draft reads as safe, it has failed the voice, not satisfied it.
