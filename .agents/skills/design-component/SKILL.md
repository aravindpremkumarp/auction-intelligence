...
> **Resource root (vendored kit):** This skill is part of the `ux-ui-agent-skills` kit vendored into this repo. All relative paths it references — e.g. `tokens/…`, `scripts/…`, `taste/…`, `workflows/…`, `accessibility/…`, `components/…`, `frameworks/…`, `content/…`, `design-systems/…`, `examples/…`, and `CLAUDE.md` — resolve under `.agents/ux-ui-kit/`. Read them from there.
